...
{
    SYSCTL->RCGCTIMER |= (1<<0); //Encender el Periferico Timer 0  pag 339
...
extern void Configurar_Timer1A(void(*task)(void), unsigned long int periodo)
{
    SYSCTL->RCGCTIMER |= (1<<1); //Encender el Periferico Timer 1  pag 339
    PeriodicTask = task; // asignar la direccion de memoria de la tarea a temporizar
    ...
    // seleciona 32 bits con 0x0
    TIMER1->TAMR = (0x2<<0); // modo periodico pag 981
